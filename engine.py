...
class Game_state():
    ...
    def get_knight_moves(self, r, c, moves):
        if self.light_to_move:
             if r > 1:
                 if c > 0:
                     if self.board[r-2][c-1] == "  " or self.board[r-2][c-1][1] == "d":
             ...
             if r != 0:
                 if c > 1:
                     if self.board[r-1][c-2] == "  " or self.board[r-1][c-2][1] == "d":
             ...
             if r < 6:
                ...
                if c != 7:
                    if self.board[r+2][c+1] == "  " or self.board[r+2][c+1][1] == "d":
                         moves.append(Move((r, c), (r+2, c+1), self.board))
        else:           # nd turn to move
            if r > 1:
                if c > 0:
                    if self.board[r-2][c-1] == "  " or self.board[r-2][c-1][1] == "l":
                        moves.append(Move((r, c), (r-2, c-1), self.board))
                if c < 7:
                    if self.board[r-2][c+1] == "  " or self.board[r-2][c+1][1] == "l": 
                        moves.append(Move((r, c), (r-2, c+1), self.board))
            if r != 0:
                if c > 1:
                    if self.board[r-1][c-2] == "  " or self.board[r-1][c-2][1] == "l":
                        moves.append(Move((r, c), (r-1, c-2), self.board))
                if c < 6:
                    if self.board[r-1][c+2] == "  " or self.board[r-1][c+2][1] == "l":
                        moves.append(Move((r, c), (r-1, c+2), self.board))
