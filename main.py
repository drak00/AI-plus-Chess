        ...
        if not user_prompt:
            ...
            for e in pg.event.get():
                if e.type == pg.QUIT:
                    ...
                    # u key pressed (undo last move)
                    if e.key == pg.K_u and not AI_MODE:
                        gs.undo_move()
                        valid_moves, first_click_turn = gs.get_valid_moves()
                                            ...
                                            while promotion:
                                                for event in pg.event.get():
                                                    if event.type == pg.MOUSEBUTTONDOWN:
                                                        location = pg.mouse.get_pos()  # x, y location of mouse click
                                                        # divided by sqaure size
                                                        location_col_transform = location[0] // square - 1
                                                        location_row_transform = location[1] // square - 1
                                                        # list for all the possible rows
                                                        row_comb = [
                                                            1, 2, 15, 16]
                                                        if location_row_transform in row_comb:  # making sure the event meet requirements
                                                            ...
                                                            promotion = False
                                                display_board2(
                                                    screen, move.end_row, move.end_col)
                                                pg.display.update()
                                                clock.tick(5)

                                            gs.board[move.end_row][move.end_col] = piece + \
                                                move.piece_moved[1]
                                            user_prompt = False
